...
## 勝率の公式定義（プロジェクト全体で統一・変更禁止）

本プロジェクトにおける「勝ち」「勝率」の定義:
...
- **勝率** = RR≥1.0の取引数 / 全取引数
- **集計期間**: 30取引ローリング / デイリー / ウィークリー / マンスリー × 銘柄別 + 総合
- **「勝ち」という表現は上記の正式定義以外での使用を全面禁止する**
...
## ブランチ運用ルール
- **新しいセッションで作業を開始するとき、必ず feature ブランチを切ってから着手する**
- 命名規則: `feature/YYYYMMDD-作業内容`（例: `feature/20260322-trailing-stop`）
- 作業完了後: `gh pr create` で PR を作成 → セルフマージ
- `master` に直接 push しない（PRテンプレ: `.github/PULL_REQUEST_TEMPLATE.md`）

## 修正後の自己確認ルール（省略禁止）
- **UI/フロントエンドを修正したら、必ずデプロイ後にChrome DevTools MCPでスクリーンショットを撮って自分で確認する**
- ユーザーに「確認してください」と言う前に、自分で画面を見て問題がないことを確認する
