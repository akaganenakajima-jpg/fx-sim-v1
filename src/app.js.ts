...
(function () {
  ...
  function renderStatsTab(data) {
    ...
    // RR帯別内訳
    var rrBdEl = el('rr-breakdown');
    if (rrBdEl) {
      var rrBd = data.rrBreakdown;
      if (!rrBd) {
        ...
        function fmtBdRow(label, color, tier) {
          ...
          var pnlStr  = pnlSign + '¥' + Math.abs(Math.round(tier.totalPnl)).toLocaleString();
          var pnlColor = tier.totalPnl >= 0 ? 'var(--green)' : 'var(--red)';
          return '<div style="display:flex;align-items:center;justify-content:space-between;padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.05)">' +
            '<span style="font-size:12px;font-weight:600;color:' + color + ';min-width:130px">' + label + '</span>' +
            '<span style="font-size:12px;color:var(--secondary);min-width:72px;text-align:right">' + tier.n + '件 (' + pct + ')</span>' +
            '<span style="font-size:12px;color:' + pnlColor + ';min-width:80px;text-align:right">' + pnlStr + '</span>' +
  ...
  // 取引履歴ソート状態（'closed' = 決済順, 'entry' = エントリー順）
  var thSortMode = 'closed';
  window.setThSort = function(mode) {
    thSortMode = mode;
    // ボタンのactive切り替え
    var btns = document.querySelectorAll('.th-sort-btn');
    for (var i = 0; i < btns.length; i++) {
      btns[i].classList.toggle('th-sort-active', btns[i].getAttribute('data-sort') === mode);
    }
    // 再レンダリング
  ...
  function renderTradeHistory(data) {
    ...
    if (!container) return;
    var trades = (data.tradeHistory || []).slice(); // コピー
    if (trades.length === 0) {
    ...
    for (var i = 0; i < trades.length; i++) {
      ...
      html +=
        '<div style="background:var(--surface);border-radius:10px;padding:10px 12px;border-left:3px solid ' + dirColor + '">' +
          ...
          '</div>' +
          '<div style="font-size:10px;color:var(--tertiary);line-height:1.4">根拠: <span style="color:var(--secondary)">' + reasoning + '</span></div>' +
        '</div>';
    }
    html += '</div>';
