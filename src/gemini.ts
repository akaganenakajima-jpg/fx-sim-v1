...
export async function newsStage1(params: {
  ...
  const userMessage = [
    ...
    ...(performanceSummary ? [
      `【過去30件の実績サマリー】`,
      `勝率(RR>=1.0): ${(performanceSummary.winRate * 100).toFixed(0)}% / 平均RR: ${performanceSummary.avgRR.toFixed(2)}`,
      ...(performanceSummary.recentStreak <= -3 ? [`⚠️ ${Math.abs(performanceSummary.recentStreak)}連敗中 — 確信度の高いシグナルのみ推奨`] : []),
  ...
  const systemPrompt =
    ...
    '\n- 【市場の織り込み度（Priced-in）チェック — 必須】大きな値動きや週末ギャップ後のニュースは「すでに市場が織り込んだ情報」の可能性がある。判断基準: (1)ニュースが週末〜48時間前に発生したものか (2)現在レートが既に大きく動いた後か (3)テクニカル環境認識のギャップ警告が含まれているか。これらに該当する場合、同方向エントリーは逆転リスクが高い。確信度が中程度以下ならtrade_signalsから除外しHOLDを選択すること';
