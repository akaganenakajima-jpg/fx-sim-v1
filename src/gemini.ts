...
export async function newsStage1(params: {
  ...
  const systemPrompt =
    ...
    '- 【RR比設定手順・必須・自動拒否回避】①SL距離を決める（USD/JPYなら0.3〜0.8円推奨）②TP距離=SL距離×1.5倍以上を計算③絶対価格に変換して返す。例: USD/JPY SELL entry=158.37→SL距離0.4円→sl_rate=158.77, TP距離0.6円→tp_rate=157.77（RR=1.5）。RR<1.0はシステムが即自動拒否する\n' +
