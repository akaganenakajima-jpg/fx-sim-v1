...
export async function newsStage1(params: {
  ...
  const systemPrompt =
    ...
    '- 【RR比設定手順・必須・自動拒否回避】①SL距離を決める（最低tpSlMin以上必須・未満は即自動拒否。USD/JPYなら最低0.2円・推奨0.3〜0.8円）②TP距離=SL距離×1.5倍以上を計算③絶対価格に変換して返す。例: USD/JPY SELL entry=158.37→SL距離0.4円→sl_rate=158.77, TP距離0.6円→tp_rate=157.77（RR=1.5）。RR<1.0も自動拒否\n' +
