...
JSON配列のみを返し、他の文字は一切含めないでください。`;
    ...
    const res = await fetch(
      // gemini-2.0-flash: thinking なし・安全フィルター緩め・地政学ニュースを正常に処理できる
    ...
    if (data.usageMetadata) {
      void insertTokenUsage(db, 'gemini-2.0-flash', 'NEWS_FILTER',
...
JSON配列のみを返し、他の文字は一切含めないでください。`;
    ...
    if (data.usageMetadata) {
      void insertTokenUsage(db, 'gemini-2.0-flash', 'NEWS_TRANSLATE',
        data.usageMetadata.promptTokenCount ?? 0, data.usageMetadata.candidatesTokenCount ?? 0);
    }
