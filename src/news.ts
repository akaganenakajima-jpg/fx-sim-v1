...
JSON配列のみを返し、他の文字は一切含めないでください。`;
    ...
    const res = await fetch(
      // gemini-2.0-flash-lite: thinking なし・安全フィルター緩め・地政学ニュースを正常に処理できる
      // gemini-2.0-flash は Google が廃止（"no longer available to new users"） → lite版に移行
      // gemini-2.5-flash は thinking mode + 安全フィルターが戦争・軍事記事を全ブロックする問題あり
    ...
    if (data.usageMetadata) {
      void insertTokenUsage(db, 'gemini-2.0-flash-lite', 'NEWS_FILTER',
...
JSON配列のみを返し、他の文字は一切含めないでください。`;
    ...
    if (data.usageMetadata) {
      void insertTokenUsage(db, 'gemini-2.0-flash-lite', 'NEWS_TRANSLATE',
        data.usageMetadata.promptTokenCount ?? 0, data.usageMetadata.candidatesTokenCount ?? 0);
    }
